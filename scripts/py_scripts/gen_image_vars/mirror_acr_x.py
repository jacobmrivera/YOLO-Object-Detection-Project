...
Description:
    Mirror image and annotation file across x axis
'''
...
def main():
    ...
    if output_img is None:
        output_img = input_img.replace(".jpg", "_mirror_acr_x.jpg")
    if output_txt is None:
        output_txt = input_txt.replace(".txt", "_mirror_acr_x.txt")

    gen_img_vars.flip_image_vertically(input_img, output_img)

    # this line could be placed in below function
    width, height = gen_img_vars.get_image_resolution(input_img)

    gen_img_vars.process_text_file(input_txt, output_txt, width, height, 'x')
